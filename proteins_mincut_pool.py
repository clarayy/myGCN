...
def test(loader):
    ...
    for data in loader:
        ...
        preds.append(pred.max(dim=1)[1].cpu().data.numpy())
    ...
    return loss_all / len(loader.dataset), correct / len(loader.dataset), average_dis
...
for epoch in range(1, 150):
    ...
    _, train_acc, train_GCN = test(train_loader)
    val_loss, val_acc, val_GCN = test(val_loader)
    if val_loss < best_val_loss:
        test_loss, test_acc, test_GCN = test(test_loader)
    ...
    print(f'Epoch: {epoch:03d}, Train Loss: {train_loss:.3f}, '
          ...
          f'Train_GCN:{train_GCN:.4f},Val_GCN: {val_GCN:.4f}, Test_GCN:{test_GCN:.4f}')
# for epoch in range(1, 150):
#     train_loss = train(epoch)

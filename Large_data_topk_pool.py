...
class MyDataset(Dataset):  # 创建自己的类：MyDataset,这个类是继承的torch.utils.data.Dataset
    def __init__(self, root, txt, transform=None, target_transform=None, loader=default_loader):  # 初始化一些需要传入的参数
        ...
        coo_A = coo_matrix(A)  # 邻接矩阵的边的行/列的坐标
    ...
    def __getitem__(self, index):
        ...
        data = Data(x=node_labels, edge_index=self.edge_index, y=label, mask=mask)
        return data
...
train_loader = DataLoader(dataset=train_dataset, batch_size=10, shuffle=True, num_workers=1)
val_loader = DataLoader(dataset=val_dataset, batch_size=10, shuffle=True, num_workers=1)
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.lin3 = torch.nn.Linear(64, 236)
...
for epoch in range(1, 51):
    ...
    test_acc, test_GCN = test(val_loader)
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), 'model/' + bmname + '_topk_pool.pt')
torch.save(model.state_dict(), 'model/' + bmname + '_topk_pool.pt')